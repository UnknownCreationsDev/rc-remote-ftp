...
import RcView from './views/rc-view'
...
export default {
  ...
  activate(state) {
    ...
    if (this.subscriptions) this.deactivate();

    this.config = config;
    this.client = new Client();
    atom.project.rcRemoteftpMain = this;
    atom.project.rcRemoteftp = this.client;
    this.treeView = new TreeView(this.storage);
    this.rcView = new RcView(this.storage);

    this.subscriptions = new CompositeDisposable();
    this.subscriptions.add(
      atom.workspace.observeTextEditors((editor) => {
        this.subscriptions.add( editor.onDidSave(event => this.fileSaved(event)) );
      }),

      atom.project.onDidChangePaths(() => {
        if (!hasProject() || !this.client.isConnected()) return;

        atom.commands.dispatch(atom.views.getView(atom.workspace), 'rc-remote-ftp:disconnect');
        atom.commands.dispatch(atom.views.getView(atom.workspace), 'rc-remote-ftp:connect');
      }),
      ...
      atom.config.onDidChange('remote-ftp.tree.enableDragAndDrop', (value) => {
        if (value.newValue)  this.createTreeViewEvents();
        else this.dropTreeViewEvents();
      }),
    );

    if (atom.config.get('remote-ftp.tree.enableDragAndDrop')) this.createTreeViewEvents();
    ...
    if (hasProject()) {
      ...
      setTimeout(() => {
        ...
        conf.exists().then((exists) => {
          if (exists && atom.config.get('remote-ftp.tree.showViewOnStartup')) this.treeView.attach();
        }).catch((error) => {
          atom.notifications.addWarning((error.reason) ? error.reason : error.message);
        });
      }, 0);
    }
  ...
  createTreeViewEvents() {
    ...
    const coreTreeViewElemDrop = (e) => {
      ...
      if (pathInfos.length > 0) {
        ...
        if (Object.keys(remotePath).length === 0) console.warn('Empty ', remotePath);
    ...
    atom.packages.activatePackage('tree-view').then((treeView) => {
      ...
      if (typeof this.coreTreeView.element === 'undefined') return;
  ...
  fileSaved(text) {
    ...
    if (!this.client.ftpConfigPath) this.client.readConfig();
    ...
    this.client.upload(local, (err) => {
      if (atom.config.get('remote-ftp.notifications.enableTransfer')) {
        if (err) atom.notifications.addError(`Remote FTP: ${uploadedItem} could not upload.`);
        else atom.notifications.addSuccess(`Remote FTP: ${uploadedItem} uploaded.`);
      }
    });
  },
  ...
  consumeElementIcons(fn) {
    setIconHandler(fn);
    return new Disposable(() => setIconHandler(null) );
  },
  ...
  setStatusbar(statusBar) {
    ...
    this.subscriptions.add(
      atom.config.onDidChange('remote-ftp.statusbar.enable', () => this.setStatusbar(statusBar) ),
    );
    ...
    this.subscriptions.add(
      atom.config.onDidChange('remote-ftp.statusbar.position', () => this.setStatusbar(statusBar) ),
      atom.config.onDidChange('remote-ftp.statusbar.priority', () => this.setStatusbar(statusBar) ),
    );
  },
  ...
  consumeStatusBar(statusBar) { this.setStatusbar(statusBar); },
