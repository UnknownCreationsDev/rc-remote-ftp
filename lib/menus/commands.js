...
const init = () => {
  const client = atom.project.rcRemoteftp;
  const remoteftp = atom.project.rcRemoteftpMain;
  ...
  const commands = {
    'rc-remote-ftp:restart-apache': {
      enabled: true,
      command() {
        if(client.socketIO == null){
          //output error message
          return;
        }
        client.socketIO.socket.emit('restartApache', { })
      },
    },
    'rc-remote-ftp:create-ftp-config': {
      enabled: true,
      command() {
        createConfig({
          protocol: 'ftp',
    ...
    'rc-remote-ftp:create-sftp-config': {
      enabled: true,
      command() {
        createConfig({
          protocol: 'sftp',
    ...
    'rc-remote-ftp:create-ignore-file': {
      enabled: true,
      command() {
        if (!hasProject()) return;
    ...
    'rc-remote-ftp:toggle': {
      enabled: true,
      command() {
        remoteftp.treeView.toggle();
        remoteftp.rcView.toggle();
      },
    },
    'rc-remote-ftp:connect': {
      enabled: true,
      command() {
        if (!hasProject()) return;
    ...
    'rc-remote-ftp:refresh-selected': {
      enabled: true,
      command() {
        if (!hasProject()) return;
    ...
    'rc-remote-ftp:download-selected': {
      enabled: true,
      command() {
        if (!hasProject()) return;
    ...
    'rc-remote-ftp:download-selected-local': {
      enabled: true,
      command() {
        if (!hasProject()) return;
        ...
        if (!client.isConnected()) {
          ...
          atom.commands.dispatch(viewWorkspace, 'rc-remote-ftp:connect');

          atom.project.rcRemoteftp.onceConnected(() => {
            atom.commands.dispatch(viewWorkspace, 'rc-remote-ftp:download-selected-local');
          });
        ...
        $treeSelected.each((key, elem) => {
          ...
          const remotePath = Path.posix.normalize((atom.project.rcRemoteftp.root.remote + localPath).replace(/\\/g, '/'));
    ...
    'rc-remote-ftp:upload-selected': {
      ...
      command() {
        ...
        if (!client.isConnected()) {
          ...
          atom.project.rcRemoteftp.onceConnected(() => {
            atom.commands.dispatch(viewWorkspace, 'rc-remote-ftp:upload-selected');
          });
    ...
    'rc-remote-ftp:upload-active': {
      ...
      command() {
        ...
        client.upload(local, (err, list) => {
          ...
          const dirs = [];
          list.forEach((item) => {
            const remote = atom.project.rcRemoteftp.toRemote(item.name);
            const dir = Path.dirname(remote);
            if (dirs.indexOf(dir) === -1) dirs.push(dir);
          });
    ...
    // Remote -> Local
    'rc-remote-ftp:sync-with-remote': {
      enabled: true,
      command() {
        const remotes = remoteftp.treeView.getSelected();
    ...
    'rc-remote-ftp:sync-with-local': {
      ...
      command() {
        ...
        if (!client.isConnected()) {
          ...
          atom.project.rcRemoteftp.emitter.once('connected', () => {
            atom.commands.dispatch(viewWorkspace, 'rc-remote-ftp:sync-with-local');
          });
    ...
    'rc-remote-ftp:permission-selected': {
      enabled: true,
      command() {
        if (!hasProject()) return;
