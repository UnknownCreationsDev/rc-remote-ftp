...
class TreeView extends ScrollView {
  ...
  initialize(storage) {
    ...
    <ul>
      ...
      <li><a role="configure_rc" class="btn btn-default icon">Edit RC Configuration</a><br /></li>
    ...
    if (atom.project.rcRemoteftp.isConnected()) {
      this.showOnline();
    } else {
      this.showOffline();
    }

    this.root = new DirectoryView(atom.project.rcRemoteftp.root);
    this.root.expand();
    ...
    atom.project.rcRemoteftp.onDidDebug((msg) => {
    ...
    atom.project.rcRemoteftp.onDidQueueChanged(() => {
      this.progress.empty();

      const queues = [];
      if (atom.project.rcRemoteftp.current) {
        queues.push(atom.project.rcRemoteftp.current);
      }

      atom.project.rcRemoteftp.queue.forEach((queueElem) => {
    ...
    this.offline.on('click', '[role="connect"]', () => {
      atom.project.rcRemoteftp.readConfig(() => {
        atom.project.rcRemoteftp.connect();
      });
    });

    this.offline.on('click', '[role="configure"]', () => {
      atom.workspace.open(atom.project.rcRemoteftp.getConfigPath());
    });

    this.offline.on('click', '[role="configure_rc"]', () => {
      atom.workspace.open(atom.project.rcRemoteftp.getRcConfigPath());
    });
    ...
    atom.project.rcRemoteftp.onDidConnected(() => {
      this.showOnline();
    });

    atom.project.rcRemoteftp.onDidDisconnected(() => {
      this.showOffline();
    });
  ...
  showOnline() {
    ...
    if (!atom.project.rcRemoteftp.connector.ftp) {
      this.info.hide();
    }
  }

  remoteMultiSelect(e, current) {
    const treeView = atom.project.rcRemoteftpMain.treeView;
