...
class DirectoryView extends View {
  ...
  initialize(directory) {
    ...
    if (atom.project.rcRemoteftp.checkIgnore(this.item.remote)) {
  ...
  triggers() {
    this.subscriptions.add(
      this.item.onChangeSelect(() => {
        let lastSelected = atom.project.rcRemoteftpMain.treeView.lastSelected;
  ...
  static actionRemoteMove(e, dataTransfer) {
    const ftp = atom.project.rcRemoteftpMain;
  ...
  static actionToRemote(e, dataTransfer) {
    ...
    atom.project.rcRemoteftpMain.client.uploadTo(localPaths.fullPath, destPath, (err) => {
      if (err) console.error(err);
    });
  }
