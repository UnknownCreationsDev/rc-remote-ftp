...
import { unableToConnectFtp } from './notifications.js'
import defaultRcConfig from './defaultconf/default-rcconfig'
import defaultFtpConfig from './defaultconf/default-ftpconfig'
...
export default class Client {
  constructor() {
    ...
    self.rcConfigFileName = '.rcconfig';
  ...
  getFilePath(relativePath) {
    ...
    projectPath = Path.dirname(projectPath).split(Path.sep).pop();
    projectPath += "/rc-remote-ftp-configs/";
  ...
  getConfigPath() {
    ...
    filePath = this.getFilePath(process.env.APPDATA + `/rc-remote-ftp/${this.configFileName}`)

    if(!fs.existsSync(filePath))
      fs.writeFileSync(filePath, JSON.stringify(defaultFtpConfig, null, 2));
  ...
  getRcConfigPath() {
    if (!hasProject()) return false;
    filePath = this.getFilePath(process.env.APPDATA + `/rc-remote-ftp/${this.rcConfigFileName}`)

    if(!fs.existsSync(filePath))
      fs.writeFileSync(filePath, JSON.stringify(defaultRcConfig, null, 2));
  ...
  isConnected() { return this.connector && this.connector.isConnected(); }
  ...
  doConnect() {
    ...
    self.socketIO = new socketIO(this); //change this later
    ...
    self.connector.on('error', (err, code) => {
      ...
      unableToConnectFtp(dismissable = false, reason = err);
  ...
  _next() {
    ...
    if (typeof atom.project.rcRemoteftp.emitter !== 'undefined') {
      atom.project.rcRemoteftp.emitter.emit('queue-changed');
    }
  }
          ...
          traverseTree(local, (locals) => {
            ...
            const n = () => {
              ...
              if (!nRemote) {
                ...
                //if file is edited else where throw error saying something like: this file is being edited by: user
